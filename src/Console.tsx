...
import type { Person, PersonCore, PersonListItem, FactRow, StaffSummary } from "./types";
...
function EmploymentPanel({person,role,onSaved}:{person:Person;role:string|null;onSaved:(person:Person)=>void}){
  ...
  const save=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError("");setSuccess("");try{const payload:Record<string,unknown>={offer_text:offer};if(stageId!==(current.stage_id||""))payload.stage_id=stageId||null;onSaved(await adminRequest<Person>(`/admin/persons/${person.id}/employment`,{method:"PATCH",body:JSON.stringify(payload)}));setSuccess("Результат і пропозицію збережено")}catch(e){setError(errText(e))}finally{setBusy(false)}};
  ...
  return <div className="employment-layout">
    <form className="panel form employment-card" onSubmit={save}>
      <div className="profile-card-heading"><span className="profile-section-icon"><ProfileIcon name="spark"/></span><div><h2>Результат і пропозиція</h2><p>Короткий поточний етап і конкретні рекомендації для клієнта</p></div></div>
      ...
      <label><span>Що запропонувати</span><textarea aria-label="Що запропонувати" rows={8} maxLength={5000} disabled={busy} value={offer} placeholder="Наприклад: запропонувати вакансії адміністратора з денним графіком; допомогти оновити резюме; надіслати посилання на курс Excel…" onChange={e=>setOffer(e.target.value)}/><small className="field-counter">{offer.length} / 5000</small></label>
...
function SupportPanel({person,role,staffId,onSaved}:{person:Person;role:string|null;staffId:number|null;onSaved:(person:Person)=>void}){
  const current=person.workflow||{client_requests:[],responsible:null,needs_contact:false,next_action_text:null,next_action_at:null};
  const [requestTypes,setRequestTypes]=useState<ClientRequestType[]>([]),[consultants,setConsultants]=useState<Staff[]>([]);
  const [selectedRequests,setSelectedRequests]=useState<string[]>(current.client_requests.map(item=>item.id)),[responsibleId,setResponsibleId]=useState(current.responsible?.id?String(current.responsible.id):"");
  const [nextAction,setNextAction]=useState(current.next_action_text||""),[nextAt,setNextAt]=useState(localDateTimeValue(current.next_action_at)),[notes,setNotes]=useState(person.core.notes||"");
  const [newName,setNewName]=useState(""),[editingId,setEditingId]=useState<string|null>(null),[editingName,setEditingName]=useState("");
  const [error,setError]=useState(""),[success,setSuccess]=useState(""),[busy,setBusy]=useState(false);
  const canManage=role==="admin"||role==="super_admin";
  ...
  useEffect(()=>{load().catch(e=>setError(errText(e)))},[]);
  useEffect(()=>{setSelectedRequests(current.client_requests.map(item=>item.id));setResponsibleId(current.responsible?.id?String(current.responsible.id):"");setNextAction(current.next_action_text||"");setNextAt(localDateTimeValue(current.next_action_at));setNotes(person.core.notes||"")},[person.id,current.responsible?.id,current.next_action_text,current.next_action_at,person.core.notes]);
  const toggleRequest=(id:string)=>setSelectedRequests(values=>values.includes(id)?values.filter(value=>value!==id):[...values,id]);
  const save=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError("");setSuccess("");if(Boolean(nextAction.trim())!==Boolean(nextAt)){setError("Для наступної дії вкажіть і опис, і дату");setBusy(false);return}try{const payload={client_request_ids:selectedRequests,responsible_staff_id:responsibleId?Number(responsibleId):null,next_action_text:nextAction.trim()||null,next_action_at:nextAt?new Date(nextAt).toISOString():null,notes:notes.trim()||null};onSaved(await adminRequest<Person>(`/admin/persons/${person.id}/workflow`,{method:"PATCH",body:JSON.stringify(payload)}));setSuccess("Супровід клієнта оновлено")}catch(e){setError(errText(e))}finally{setBusy(false)}};
  ...
  return <section className="client-profile simplified-profile">
    ...
    <div hidden={tab!=="core"}><div className="profile-overview">
      ...
      <section className="panel profile-contact-card profile-contact-summary">
        <div className="profile-card-heading"><span className="profile-step-number">01</span><div><h2>Особисті дані</h2><p>Контакти та місце проживання</p></div>{!editingContacts&&<button className="profile-edit-link" type="button" onClick={()=>setEditingContacts(true)}>Редагувати</button>}</div>
      ...
      <section className="profile-ability-section profile-section-surface"><div className="profile-card-heading"><span className="profile-step-number">02</span><div><h2>Професійний профіль</h2><p>Освіта, досвід роботи та навички</p></div></div>
      ...
      <section className="profile-section-surface"><div className="profile-card-heading"><span className="profile-step-number">03</span><div><h2>Яка робота підходить</h2><p>Практичні умови для підбору вакансій</p></div></div><ProfileFold title="Умови роботи та мобільність" hint="Формат роботи, готовність до переїзду, права й авто"><MobilityEditor person={person} onSaved={accept}/></ProfileFold></section>
      </div>
      <aside className="profile-sidebar">
        <section className="profile-result-card"><span className="console-kicker">ПОТОЧНИЙ РЕЗУЛЬТАТ</span><h3>{person.employment?.stage_name||"Етап не вказано"}</h3><p className={person.employment?.offer_text?"profile-note-preview":""}>{person.employment?.offer_text||"Додайте етап працевлаштування та запишіть, що запропонувати клієнту."}</p><button type="button" className="profile-text-action" onClick={()=>setTab("employment")}>{person.employment?.stage_id||person.employment?.offer_text?"Відкрити результат":"Додати результат"}<ProfileIcon name="arrow"/></button></section>
        <section className="profile-followup-card"><span className="console-kicker">РОБОТА З КЛІЄНТОМ</span><h3>{person.workflow?.client_requests.length?person.workflow.client_requests.map(item=>item.name).join(" · "):"Запит ще не визначено"}</h3><p>{person.workflow?.responsible?<><b>{person.workflow.responsible.full_name||person.workflow.responsible.email}</b><br/></>:"Без відповідального консультанта. "}{person.workflow?.next_action_at?`Наступна дія: ${new Date(person.workflow.next_action_at).toLocaleString("uk-UA",{day:"2-digit",month:"long",hour:"2-digit",minute:"2-digit"})}`:"Наступну дію не заплановано."}</p><button type="button" className="profile-text-action" onClick={()=>setTab("support")}>{person.workflow?.client_requests.length||person.workflow?.responsible?"Відкрити супровід":"Налаштувати супровід"}<ProfileIcon name="arrow"/></button></section>
